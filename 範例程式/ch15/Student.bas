...
Sub btnInsert_Click  '新增「學生」資料
 ...
 '判斷使用者是否有完整的填入三個欄位內容
 If StudNo="" OR StudName="" Then 
    Msgbox("您尚未完整輸入「學生資料」哦！","錯誤訊息回報!!!")
 Else
    strSQL = "INSERT INTO 學生資料表(學號,姓名)" & _
             "VALUES('" & StudNo & "','" & StudName & "')"
    Check_StuID_Exists(StudNo)        '呼叫檢查「學號」是否有重複新增之副程式
...
Sub btnUpdate_Click   '修改「學生」資料
 ...
 Else
   ...
   Dim strSQL As String
   strSQL = "UPDATE 學生資料表 SET 姓名 ='" & edtStudName.Text & "' WHERE 學號 = '" & StudNo & "'"
...
Sub SpinQueryStudNo_ItemClick (Position As Int, Value As Object)
	...
	SQL_Query="Select * FROM 學生資料表 Where 學號 = '" & QueryDeptNo & "'"
