...
Sub Activity_Resume
    If WriteSQL.YourSQL.Length > 0 Then  '判斷剛才是否有自行撰寫「SQL」指令,如果有
	   edtSQL.Text =""                   '先將原來的SQL區塊清空
       edtSQL.Text =WriteSQL.YourSQL     '取得最新撰寫的「SQL」指令
    End If
End Sub
...
Else
   QueryDBList(edtSQL.Text) '呼叫顯示查詢結果
End If
End Sub
Sub QueryDBList(strSQLite As String)  '定義「顯示查詢結果」之副程式
    livSQLView.Clear    　 '清單元件先清空
	strSqlViewTitle =""　　'清單內的「欄位名稱」清空
	strSqlViewContent =""　'清單內的「欄位內容」清空
	strLine =""
	...
	Next	
    livSQLView.AddSingleLine (strSqlViewTitle)  '顯示「欄位名稱u」
	...
	lblResult.Text ="==符合條件的記錄共有( " & CursorDBPoint.RowCont & " 筆)=="
End Sub
