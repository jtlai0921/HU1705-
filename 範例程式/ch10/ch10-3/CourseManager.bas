...
Sub btnInsert_Click  '新增「課程資料」
 ...
 If CourNo="" OR CourName="" OR CourCredits="" Then 
    Msgbox("您尚未完整輸入「課程資料」哦！","錯誤訊息回報!!!")
 Else
    strSQL = "INSERT INTO 課程資料表(課號,課名,學分數)" & _
             "VALUES('" & CourNo & "','" & CourName & "'" & _
			 ",'" & CourCredits & "')"
    Check_CourID_Exists(CourNo)        '呼叫檢查「課程資料」是否有重複新增之副程式
	If Check_CourID_Exist=True Then    '檢查是否有重複新增
	    Msgbox("您已經重複新增「課程資料」了！","新增錯誤訊息!!!")
    Else                               '沒有重複新增
	  SQLCmd.ExecNonQuery(strSQL)                             '執行SQL指令，以成功的新增記錄
      ToastMessageShow("您新增成功「課程資料」記錄!", True)   '顯示新增成功狀態
    End If
	Check_CourID_Exist=False           '設定沒有重複新增「課程資料」
  End If
  QueryDBList("Select * FROM 課程資料表")
End Sub

Sub Check_CourID_Exists(strCourNo As String)  '檢查「課程資料」是否有重複新增之副程式
	...
	If CursorDBPoint.RowCount>0 Then  '檢查目前資料庫中的「課程資料表」是否已經存在一筆了
	   Check_CourID_Exist=True        '代表重複新增「課程資料」了!
	End If
End Sub
